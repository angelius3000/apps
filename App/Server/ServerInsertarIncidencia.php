...
$creadorSeleccionado = trim((string)($_POST['aduanaId'] ?? ''));
$creadorOtro = trim((string)($_POST['creadorOtro'] ?? ''));
...
if ($folio === '' || (!$productoId && !$esProductoSolicitado) || $responsableSeleccionado === '' || ($responsableSeleccionado === 'otro' && $responsableOtro === '') || $creadorSeleccionado === '' || ($creadorSeleccionado === 'otro' && $creadorOtro === '') || $cantidad === false || $cantidad <= 0 || $precio === false || $precio < 0) { responderIncidencia(['ok' => false, 'error' => 'Completa todos los campos obligatorios con valores válidos.'], 422); }
$stmtProducto = mysqli_prepare($conn, 'SELECT Sku, Descripcion, MarcaProductos FROM productos WHERE PRODUCTOSID = ? LIMIT 1');
if (!$stmtProducto) { responderIncidencia(['ok' => false, 'error' => 'No se pudo validar la información seleccionada.'], 500); }
...
$creadoPor = $creadorOtro;
if ($creadorSeleccionado !== 'otro') {
    if (!ctype_digit($creadorSeleccionado) || (int)$creadorSeleccionado < 1) { responderIncidencia(['ok' => false, 'error' => 'El creador seleccionado no es válido.'], 422); }
    $stmtAduana = mysqli_prepare($conn, 'SELECT NombreAduana FROM aduana WHERE AduanaID = ? AND Deshabilitado = 0 LIMIT 1');
    if (!$stmtAduana) { responderIncidencia(['ok' => false, 'error' => 'No se pudo validar el creador seleccionado.'], 500); }
    $aduanaId = (int)$creadorSeleccionado;
    ...
    mysqli_stmt_close($stmtAduana);
    $creadoPor = (string)($aduana['NombreAduana'] ?? '');
}
...
if (!$producto || $responsable === '' || $creadoPor === '') { responderIncidencia(['ok' => false, 'error' => 'Alguna de las opciones seleccionadas ya no está disponible.'], 422); }
...
$sku = (string)$producto['Sku']; $descripcion = (string)$producto['Descripcion']; $marca = (string)($producto['MarcaProductos'] ?? '');
