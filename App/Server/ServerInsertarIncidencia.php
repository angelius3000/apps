...
if ($folio === '' || !$productoId || !$vendedorId || !$aduanaId || $cantidad === false || $cantidad <= 0 || $precio === false || $precio < 0) { responderIncidencia(['ok' => false, 'error' => 'Completa todos los campos obligatorios con valores válidos.'], 422); }
...
mysqli_stmt_bind_param($stmtProducto, 'i', $productoId); mysqli_stmt_execute($stmtProducto); $producto = mysqli_fetch_assoc(mysqli_stmt_get_result($stmtProducto));
