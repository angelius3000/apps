...
$stmtFacturaActual = mysqli_prepare(
    $conn,
    'SELECT DocumentoFMP FROM facturamp WHERE FacturaMPID = ? LIMIT 1'
);
...
$stmtActualizar = mysqli_prepare(
    $conn,
    'UPDATE facturamp SET DocumentoFMP = ?, RazonSocialFMP = ?, VendedorFMP = ?, SurtidorFMP = ?, ClienteFMP = ?, AduanaFMP = ? WHERE FacturaMPID = ? LIMIT 1'
);
